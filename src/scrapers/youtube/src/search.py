...
from src.dedup import PIPELINE_STATUS_OPTIONS, VideoRegistry
from src.utils import (
    append_jsonl,
    hash_config_section,
    language_hint_from_query_language,
    load_jsonl,
    project_root,
    resolve_path,
    retry_with_backoff,
...
def candidate_to_registry_fields(row: dict[str, Any], search_query: str, status: str) -> dict[str, Any]:
    ...
    return {
        ...
        "duration_sec": int(float(row.get("duration_seconds") or 0)),
        "published_at": str(row.get("published_at", "")),
        "language_hint": language_hint_from_query_language(row.get("query_language")),
        "behavioral_category": str(row.get("behavioral_category", "")),
        "query_that_found_it": search_query,
        "pipeline_status": status,
        "pipeline_status_options": PIPELINE_STATUS_OPTIONS,
        "snippets_count": int(row.get("snippets_count") or 0),
...
def resolve_seed_query_limits(search_cfg: dict[str, Any]) -> dict[str, int]:
    """
    Per-category English seed counts. Backward-compatible: if only ``seed_queries_per_category``
    is set, all five categories use that value. Optional ``seed_queries_per_category_pain_vet``
    sets Pain/Vet only; optional ``seed_queries_per_category_by_category`` overrides any key.
    """
    ...
    if isinstance(by_cat, dict) and by_cat:
        out: dict[str, int] = {}
        for k in BEHAVIORAL_CATEGORY_ORDER:
    ...
    if n_pv is not None:
        try:
            ...
            n_pv_i = base
        return {k: (n_pv_i if k == "Pain/Vet" else base) for k in BEHAVIORAL_CATEGORY_ORDER}
    return {k: base for k in BEHAVIORAL_CATEGORY_ORDER}
...
def _generate_seed_queries_openai(
    ...
    system = (
        ...
        "cats exhibiting the following behavioral categories. "
        ...
        "Respect the **exact per-category counts** in the user message (Pain/Vet is the largest "
        "bucket when specified). "
    ...
    lines = [
        f"- {cat}: {limits[cat]} distinct queries" for cat in BEHAVIORAL_CATEGORY_ORDER
    ]
...
def load_or_generate_queries(
    ...
    root = project_root()
    search_cfg = cfg["search"]
    cache_rel = search_cfg.get("query_cache_file", "logs/generated_queries.jsonl")
    cache_path = resolve_path(root, cache_rel)
    cache_path.parent.mkdir(parents=True, exist_ok=True)
    meta_path = cache_path.parent / (cache_path.stem + ".meta.json")
...
def run_search(
    ...
    prior_skip: set[str] = registry.build_skip_set() if registry else set()
    if prior_skip:
        seen_ids.update(prior_skip)
        logger.info(
            "Search: excluding %d video_id(s) from registry skip set (no duplicate work vs saved runs).",
            len(prior_skip),
        )
        ...
        for row in rows:
            ...
            if vid in seen_ids:
                ...
                if registry:
                    registry.upsert(
                        str(vid),
                        candidate_to_registry_fields(row, q, "skipped_duplicate"),
                    )
                continue
            seen_ids.add(vid)
            ...
            if registry:
                registry.upsert(
                    str(vid),
                    candidate_to_registry_fields(row, q, "candidate"),
                )
    ...
    if lang_unique:
        ...
        counts = [len(lang_unique[lang]) for lang in langs]
