...
def _rows_with_existing_media(rows: list[dict[str, Any]], logger: logging.Logger) -> list[dict[str, Any]]:
    ...
    if n:
        logger.warning(
            "Skipping %d video(s): source file(s) not found under dataset/videos_v2/ "
...
def _pool_init(
    ...
    """Load YOLO + AudioClassifier once per worker process (spawn), not per task."""
...
def process_batch(
    ...
    snippets_dir = resolve_path(root, out_cfg.get("snippets_dir", "data/dataset/snippets_v2"))
    metadata_file = resolve_path(root, out_cfg.get("metadata_file", "data/dataset/metadata_v2.jsonl"))
    ycfg = cfg.get("yolo", {})
