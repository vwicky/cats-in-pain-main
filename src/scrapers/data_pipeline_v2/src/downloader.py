...
def collect_skip_video_ids(cfg: dict) -> set[str]:
    """Union: v2 metadata, pipeline log, legacy final_dataset.jsonl."""
    root = project_root()
    out = cfg.get("output", {})
    meta = resolve_path(root, out.get("metadata_file", "data/dataset/metadata_v2.jsonl"))
    plog = resolve_path(root, out.get("pipeline_log", "logs/v2_pipeline_log.jsonl"))
    legacy = resolve_path(root, cfg.get("download", {}).get("legacy_dataset_jsonl", "data/dataset/final_dataset.jsonl"))
    ...
    for row in load_jsonl(plog):
        ...
        # Do not skip failed downloads so they can be retried; skip successful/complete ingest.
    ...
    if legacy.is_file():
        ...
        logging.getLogger("pipeline_v2").warning("Legacy dataset file not found: %s", legacy)
    return skip
...
def download_batch(
    ...
    root = project_root()
    dc = cfg.get("download", {})
    tmp_dir = resolve_path(root, dc.get("tmp_dir", "src/scrapers/data_pipeline_v2/tmp_downloads"))
