"""Logging, config, paths, JSONL, retries, funnel plots — TikTok pipeline."""
...
def project_root() -> Path:
    """Repository root: ``parents[4]`` of ``src/scrapers/tiktok/src/utils.py``."""
    return Path(__file__).resolve().parents[4]


def pipeline_package_root() -> Path:
    """src/scrapers/tiktok/ directory (config, cache)."""
...
def setup_logger(run_dir: str | Path, name: str = "tiktok_pipeline") -> logging.Logger:
...
def make_run_dir(cfg: dict[str, Any], run_name: str | None = None) -> Path:
    """Create tiktok_pipeline/runs/{name}_{timestamp}/ with stage subdirs."""
    root = project_root()
    name = run_name or cfg.get("run_name", "run")
    from datetime import datetime

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    runs_parent = resolve_path(root, cfg.get("output", {}).get("runs_dir", "src/scrapers/tiktok/runs"))
    ...
    with open(path, encoding="utf-8") as f:
        for line_no, line in enumerate(f, 1):
            ...
            try:
                ...
                logging.getLogger("tiktok_pipeline").warning(
...
def save_funnel_plot(stage_counts: dict[str, Any], run_dir: str | Path) -> Path:
    ...
    ax.set_xlabel("Count")
    ax.set_title("TikTok pipeline funnel (out per stage)")
