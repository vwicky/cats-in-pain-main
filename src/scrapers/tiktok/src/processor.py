"""YOLO tracking + chunking + audio — same logic as data_pipeline_v2/processor.py; TikTok paths."""
...
def load_video_ids_with_saved_snippets(metadata_paths: list[Path]) -> set[str]:
    """Video IDs that already have at least one snippet recorded in metadata (skip full re-process)."""
    out: set[str] = set()
    for mp in metadata_paths:
        if not mp.is_file():
            continue
        for row in load_jsonl(mp):
            ...
            if vid and isinstance(snippets, list) and len(snippets) > 0:
...
_EST_YOLO_RAM_MB = 130
_EST_AUDIO_RAM_MB = 50


def _rows_with_existing_media(
    rows: list[dict[str, Any]], logger: logging.Logger, out_dir_hint: str
) -> list[dict[str, Any]]:
    ...
    if n:
        logger.warning(
            "Skipping %d video(s): source file(s) not found under %s "
            "(often removed after an earlier process run; re-download with --start-from download, "
            "or set yolo.delete_source_media_after_processing: false before processing).",
            n,
            out_dir_hint,
        )
    return out
...
_POOL_AC: AudioClassifier | None = None
...
def _pool_init(
    ...
    _POOL_AC = AudioClassifier(cfg, _POOL_LOG)
...
def process_batch(
    ...
    snippets_dir = resolve_path(root, out_cfg.get("snippets_dir", "data/dataset/tiktok_snippets"))
    metadata_file = resolve_path(root, out_cfg.get("metadata_file", "data/dataset/tiktok_metadata.jsonl"))
    dl_out = resolve_path(root, cfg.get("download", {}).get("output_dir", "data/dataset/tiktok_videos"))
    ycfg = cfg.get("yolo", {})
    ...
    weights = ycfg.get("weights", "models/yolo/yolov8x.pt")
    ...
    successes = _rows_with_existing_media(successes, logger, str(dl_out))

    skip_meta = bool(ycfg.get("skip_videos_already_in_metadata", True))
    if skip_meta:
        already_done = load_video_ids_with_saved_snippets(
            [metadata_file, run_dir / "stage_5_process" / "metadata.jsonl"]
        )
        before_n = len(successes)
        successes = [
            d
            for d in successes
            if str(d.get("video_id", "")).strip() not in already_done
        ]
        n_skip = before_n - len(successes)
        logger.info(
            "Metadata with ≥1 snippet: %d unique video_id(s) (%s + run stage_5_process/metadata.jsonl).",
            len(already_done),
            metadata_file.name,
        )
        if n_skip:
            logger.info(
                "Skipping %d video(s) that still have source media but are already in metadata "
                "(yolo.skip_videos_already_in_metadata=true). "
                "Set yolo.skip_videos_already_in_metadata: false to force re-processing.",
                n_skip,
            )
        elif already_done:
            logger.info(
                "Metadata skip removed 0 videos: none of the %d metadata-backed IDs overlap the "
                "current queue (videos with files on disk). Usually those clips were already processed "
                "and source MP4/MP3 were deleted (yolo.delete_source_media_after_processing), so they "
                "were excluded earlier — the progress total is only downloads that still have files.",
                len(already_done),
            )

    logger.info("YOLO weights (effective): %s", wpath_abs)
